...
void assign_start_places(void) // randomly determines where all players will start on the board
{
    for (int i=0; i<MAX_PLAYERS; i++) {
        ...
        players[i].postion = rand() % MAX_SQUARES - 1;
...
void choose_murderer(void) // pick out a character, room and weapon for the solution
{
    ...
    int character = (rand() % 3) + 11; // characters start at 11, so add 11
    int weapon = (rand() % 6) + 4; // weapons start at 4, so add 4
    int room = (rand() % 3); // rooms start at 0, so no need to add a value
...
int dice_roll(int sides) // roles a dice of a number of sides specified by user
{
    ...
    int roll = rand() % (sides - 1); // subtract 1 from sides, so a 6 sided dice is now random of 0-5
